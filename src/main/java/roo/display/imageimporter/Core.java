...
public class Core {
  ...
  static enum TypeScoping {
    UNQUALIFIED(""),
    QUALIFIED("::roo_display::");

    TypeScoping(String scope) { this.scope = scope; }
  ...
  public Core(ImportOptions options, String name) throws IOException {
    ...
    File headerFile = new File(options.getOutputHeaderDirectory(), name + ("." +
                                                                           "h"));
    File cppFile = new File(options.getOutputHeaderDirectory(), name + (".cp" +
                                                                        "p"));
  ...
  private void writeHeaderPreamble() throws IOException {
    Writer writer =
        new BufferedWriter(new OutputStreamWriter(headerFileStream));
    ...
    if (options.getEncoding() == Encoding.INDEXED1 ||
        options.getEncoding() == Encoding.INDEXED2 ||
        options.getEncoding() == Encoding.INDEXED4 ||
        options.getEncoding() == Encoding.INDEXED8) {
  ...
  public void write(String resourceName, BufferedImage image)
      throws IOException {
    ...
    File dataFile =
        new File(options.getOutputPayloadDirectory(), resourceName + ".img");
    if (options.getStorage() == Storage.SPIFFS) {
    ...
    writeCppDefinition(resourceName, dataFile.getName(), image,
                       encoder.getProperties(), bos.toByteArray(),
  ...
  private void writeHeaderDeclaration(String resourceName) throws IOException {
    Writer writer =
        new BufferedWriter(new OutputStreamWriter(headerFileStream));
    String qualified_typename =
        getCppImageTypeNameForEncoding(options, TypeScoping.QUALIFIED);

    String template = "const {TYPE}& {VAR}();\n";

    writer.write(template.replace("{TYPE}", qualified_typename)
                     .replace("{VAR}", resourceName));
    writer.flush();
  }

  private void writeCppDefinition(String resourceName, String dataFileName,
                                  BufferedImage image,
                                  Properties encoderProperties, byte[] encoded,
                                  List<Integer> palette, int xMin, int yMin,
                                  int xMax, int yMax) throws IOException {
    Writer writer = new BufferedWriter(new OutputStreamWriter(cppFileStream));
    String unqualified_typename =
        getCppImageTypeNameForEncoding(options, TypeScoping.UNQUALIFIED);
    ...
    if (!palette.isEmpty()) {
      ...
      for (int color : palette) {
        writer.write("  Color(0x{COLOR}),".replace(
      ...
      writer.write("};\n\n");
      paletteDeclaration =
          "  static Palette palette = Palette::ReadOnly({VAR}_palette, " +
          "{PALETTE_SIZE});\n";
    }
    if (options.getStorage() == Storage.SPIFFS) {
      String template =
          ...
          + ("      Box({XMIN}, {YMIN}, {XMAX}, {YMAX}), Box(0, 0, " +
             "{WIDTH_LESS_1}, {HEIGHT_LESS_1}),\n")
          + "      file, {CONSTRUCTOR});\n"
          + "  return value;\n"
          + "}\n";

      writer.write(
          template.replace("{TYPE}", unqualified_typename)
              .replace("{VAR}", resourceName)
      ...
      hexWriter.printComment(
          "Image file " + resourceName + " " + image.getWidth() + "x" +
      ...
      String template = "\n"
                        ...
                        + ("      Box({XMIN}, {YMIN}, {XMAX}, {YMAX}), " +
                           "Box(0, 0, {WIDTH_LESS_1}, {HEIGHT_LESS_1}),\n")
  ...
  private static String getCppImageTypeNameForEncoding(ImportOptions options,
                                                       TypeScoping scoping) {
    String resource = scoping.scope() + (options.getStorage() == Storage.SPIFFS
                                             ? "FileResource"
                                             : "ProgMemPtr");
    return MessageFormat.format(getCppImageTypeNameFormatForEncoding(options),
                                resource, scoping.scope());
  }

  private static String
  getCppImageTypeNameFormatForEncoding(ImportOptions options) {
    boolean rle = (options.getCompression() == Compression.RLE);
    boolean prgmem = (options.getStorage() == Storage.PROGMEM);
    switch (options.getEncoding()) {
    case ARGB8888:
      return rle ? "{1}RleImage<{1}Argb8888, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Argb8888>"
                 : "{1}SimpleImage<{0}, {1}Argb8888>";
    case ARGB6666:
      return rle ? "{1}RleImage<{1}Argb6666, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Argb6666>"
                 : "{1}SimpleImage<{0}, {1}Argb6666>";
    case ARGB4444:
      return rle ? "{1}RleImage<{1}Argb4444, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Argb4444>"
                 : "{1}SimpleImage<{0}, {1}Argb4444>";
    case RGB565:
      return rle ? "{1}RleImage<{1}Rgb565, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Rgb565>"
                 : "{1}SimpleImage<{0}, {1}Rgb565>";
    // case RGB565_ALPHA4: return "{1}Rgb565Alpha4RleImage<{0}>";
    case GRAYSCALE8:
      return rle ? "{1}RleImage<{1}Grayscale8, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Grayscale8>"
                 : "{1}SimpleImage<{0}, {1}Grayscale8>";
    // return rle ? "{1}ImageRle4bppxBiased<{1}Alpha4, {0}>" : "{1}Raster<{0},
    // {1}Alpha4>";
    case ALPHA8:
      return rle ? "{1}RleImage<{1}Alpha8, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}, {1}Alpha8>"
                 : "{1}SimpleImage<{0}, {1}Alpha8>";
    case GRAYSCALE4:
    ...
    case INDEXED1:
      ...
      : prgmem   ? "{1}ProgMemRaster<{1}Indexed1>"
                 : "{1}SimpleImage<{0}, {1}Indexed1>";
    case INDEXED2:
      return rle ? "{1}RleImage<{1}Indexed2, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Indexed2>"
                 : "{1}SimpleImage<{0}, {1}Indexed2>";
    case INDEXED4:
      return rle ? "{1}RleImage<{1}Indexed4, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Indexed4>"
                 : "{1}SimpleImage<{0}, {1}Indexed4>";
    case INDEXED8:
      return rle ? "{1}RleImage<{1}Indexed8, {0}>"
      : prgmem   ? "{1}ProgMemRaster<{1}Indexed8>"
                 : "{1}SimpleImage<{0}, {1}Indexed8>";
    case MONOCHROME:
      return prgmem ? "{1}ProgMemRaster<{1}Monochrome>"
                    : "{1}SimpleImage<{0}, {1}Monochrome>";
    default:
      return null;
    }
  }

  // private static String getCppEncodingConstructor(ImportOptions options,
  // String transparent565Color, TypeScoping scoping) {return
  ...
  private static String
  getCppEncodingConstructor(ImportOptions options,
